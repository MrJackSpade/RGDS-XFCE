...
bool ComputeRenderer::CompileShader(GLuint& shader, const std::string& source, const std::initializer_list<const char*>& defines)
{
    ...
    // Check if we are running on GLES
    // TODO: A better check might be passing down the context type or checking a gl flag
    bool isGLES = true; // For now assuming we want to try the ES path on this platform since that's where the crash is.
                        // Ideally we check: strstr((const char*)glGetString(GL_VERSION), "OpenGL ES") 
