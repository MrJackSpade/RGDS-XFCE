...
{
  m_lib_handle = dlopen("libGLESv2.so", RTLD_NOW | RTLD_GLOBAL);
  if (!m_lib_handle)
    m_lib_handle = dlopen("libGLESv2.so.2", RTLD_NOW | RTLD_GLOBAL);
    
  if (!m_lib_handle)
    Log_WarningPrintf("Failed to load libGLESv2.so and libGLESv2.so.2, some symbols might be missing");
...
void* ContextEGL::GetProcAddress(const char* name)
{
  ...
  if (proc) {
    if (strcmp(name, "glTexStorage2D") == 0) Log_InfoPrintf("Found glTexStorage2D via eglGetProcAddress: %p", proc);
    return proc;
  }
  
  if (m_lib_handle) {
    proc = dlsym(m_lib_handle, name);
    if (strcmp(name, "glTexStorage2D") == 0) Log_InfoPrintf("Found glTexStorage2D via dlsym: %p", proc);
    return proc;
  }

  if (strcmp(name, "glTexStorage2D") == 0) Log_ErrorPrintf("Failed to find glTexStorage2D");
  return nullptr;
}
