                                             ...
                                             size_t num_versions_to_try)
{
  ...
  // Hack: glad doesn't load compute shader functions for GLES 3.1 because it thinks it's Desktop GL 3.1 (where they don't exist).
  // We manually load them here if we are in GLES mode.
  if (context->IsGLES())
  {
    Log_InfoPrintf("Manually loading Compute Shader functions for GLES...");
    auto LoadProc = [&](const char* name) { return context->GetProcAddress(name); };
    
    if (!glad_glTexStorage2D) glad_glTexStorage2D = (PFNGLTEXSTORAGE2DPROC)LoadProc("glTexStorage2D");
    if (!glad_glDispatchCompute) glad_glDispatchCompute = (PFNGLDISPATCHCOMPUTEPROC)LoadProc("glDispatchCompute");
    if (!glad_glBindImageTexture) glad_glBindImageTexture = (PFNGLBINDIMAGETEXTUREPROC)LoadProc("glBindImageTexture");
    if (!glad_glMemoryBarrier) glad_glMemoryBarrier = (PFNGLMEMORYBARRIERPROC)LoadProc("glMemoryBarrier");
    if (!glad_glShaderStorageBlockBinding) glad_glShaderStorageBlockBinding = (PFNGLSHADERSTORAGEBLOCKBINDINGPROC)LoadProc("glShaderStorageBlockBinding");
    
    // Texture Buffer functions (GLES 3.2 or EXT_texture_buffer)
    if (!glad_glTexBuffer) {
        glad_glTexBuffer = (PFNGLTEXBUFFERPROC)LoadProc("glTexBuffer");
        if (!glad_glTexBuffer) glad_glTexBuffer = (PFNGLTEXBUFFERPROC)LoadProc("glTexBufferEXT");
        if (!glad_glTexBuffer) glad_glTexBuffer = (PFNGLTEXBUFFERPROC)LoadProc("glTexBufferOES");
    }
    
    // Sampler Object functions (GL 3.3+ in Desktop, but 3.0+ in ES)
    if (!glad_glGenSamplers) glad_glGenSamplers = (PFNGLGENSAMPLERSPROC)LoadProc("glGenSamplers");
    if (!glad_glDeleteSamplers) glad_glDeleteSamplers = (PFNGLDELETESAMPLERSPROC)LoadProc("glDeleteSamplers");
    if (!glad_glBindSampler) glad_glBindSampler = (PFNGLBINDSAMPLERPROC)LoadProc("glBindSampler");
    if (!glad_glSamplerParameteri) glad_glSamplerParameteri = (PFNGLSAMPLERPARAMETERIPROC)LoadProc("glSamplerParameteri");
    if (!glad_glSamplerParameterf) glad_glSamplerParameterf = (PFNGLSAMPLERPARAMETERFPROC)LoadProc("glSamplerParameterf");

    if (glad_glTexStorage2D) Log_InfoPrintf("Successfully loaded glTexStorage2D");
    else Log_ErrorPrintf("Failed to load glTexStorage2D even with manual check");
    
    if (glad_glGenSamplers) Log_InfoPrintf("Successfully loaded glGenSamplers");
    else Log_ErrorPrintf("Failed to load glGenSamplers");
  }
