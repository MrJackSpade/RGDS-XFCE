...
const char* kScreenVS_OSD = R"(#version 310 es
...
void main()
{
    ...
    vec2 osdpos = vPosition * vec2(float(uOSDSize.x), float(uOSDSize.y));
    fTexcoord = osdpos * uTexScale;
    osdpos += vec2(float(uOSDPos.x), float(uOSDPos.y));
...
const char* kScreenFS_OSD = R"(#version 310 es
precision mediump float;
precision highp int;
