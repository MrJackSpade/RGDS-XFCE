...
//SIGHUP means we should show the configuration window.
void catchSIGHUP( int sig ) {
    if (layoutManagerPtr) layoutManagerPtr->addNewConfig();
    //remember to catch this signal again next time.
    signal( sig, catchSIGHUP );
}


int main( int argc, char **argv )
{
    //fprintf(stderr, "DEBUG: qjoypad starting\n");
    //fflush(stderr);
        ...
        if (pidFile.open( QIODevice::ReadOnly ))
        {
            ...
            if (kill(pid,0) == 0)
            {
                ...
                if (layout.isEmpty() && !update) {
                    // Instead of showing an error, signal the existing instance to show the config window
                    kill(pid, SIGHUP);
                }
                else {
    ...
    //fprintf(stderr, "DEBUG: Creating LayoutManager with devdir=%s\n", qPrintable(devdir));
    //fflush(stderr);
    ...
    //buildJoyDevices();
    //fprintf(stderr, "DEBUG: Calling updateJoyDevs\n");
    //fflush(stderr);
