...
void LayoutManager::updateJoyDevs() {
    //fprintf(stderr, "DEBUG: updateJoyDevs called\n");
    //fflush(stderr);
    ...
    // Enumerate event* devices (evdev API only, no legacy js* support)
    QStringList devices = deviceDir.entryList(QStringList("event*"), QDir::System);
    //fprintf(stderr, "DEBUG: Found %d event devices in %s\n", devices.size(), qPrintable(devdir));
    //fflush(stderr);
    int joyIndex = 0;
    foreach (const QString &device, devices) {
        QString devpath = QString("%1/%2").arg(devdir, device);
        //fprintf(stderr, "DEBUG: Trying to add device: %s\n", qPrintable(devpath));
        //fflush(stderr);
        // Try to add as joypad - will check if it's actually a joystick
        if (tryAddEventDevice(joyIndex, devpath)) {
            //fprintf(stderr, "DEBUG: Successfully added device at index %d\n", joyIndex);
            //fflush(stderr);
            joyIndex++;
        }
    }
    //fprintf(stderr, "DEBUG: Total joystick devices found: %d\n", joyIndex);
    //fflush(stderr);
    ...
    //fprintf(stderr, "DEBUG: updateJoyDevs completed\n");
    //fflush(stderr);
}
...
bool LayoutManager::isJoystickDevice(int fd) {
    // Check if device has joystick/gamepad capabilities
    ...
    if (ioctl(fd, EVIOCGBIT(0, sizeof(evbit)), evbit) < 0) {
        //fprintf(stderr, "DEBUG:   Failed to get event bits\n");
        return false;
    }

    bool hasKey = TEST_BIT(evbit, EV_KEY);
    bool hasAbs = TEST_BIT(evbit, EV_ABS);
    //fprintf(stderr, "DEBUG:   EV_KEY=%d, EV_ABS=%d\n", hasKey, hasAbs);

    // Must support EV_KEY and EV_ABS for gamepad
    if (!hasKey || !hasAbs) {
        //fprintf(stderr, "DEBUG:   Rejected: missing required event types\n");
        return false;
    }

    // Check for gamepad buttons or joystick buttons
    if (ioctl(fd, EVIOCGBIT(EV_KEY, sizeof(keybit)), keybit) >= 0) {
        // Check for gamepad buttons (BTN_GAMEPAD range) or joystick buttons
        bool hasButtons = false;
        for (int i = BTN_JOYSTICK; i < BTN_JOYSTICK + 32; i++) {
            if (TEST_BIT(keybit, i)) {
                //fprintf(stderr, "DEBUG:   Found BTN_JOYSTICK button code: 0x%x\n", i);
                hasButtons = true;
                break;
            }
        }
        // Also check common gamepad buttons
        if (!hasButtons) {
            for (int i = BTN_SOUTH; i <= BTN_THUMBR; i++) {
                if (TEST_BIT(keybit, i)) {
                    //fprintf(stderr, "DEBUG:   Found gamepad button code: 0x%x\n", i);
                    hasButtons = true;
                    break;
                }
            }
        }
        // Also check D-pad buttons (0x220-0x227)
        if (!hasButtons) {
            for (int i = 0x220; i <= 0x227; i++) {
                if (TEST_BIT(keybit, i)) {
                    //fprintf(stderr, "DEBUG:   Found D-pad button code: 0x%x\n", i);
                    hasButtons = true;
                    break;
                }
            }
        ...
        if (!hasButtons) {
            //fprintf(stderr, "DEBUG:   Rejected: no gamepad buttons found\n");
            return false;
        }
    }

    // Check for analog axes
    bool hasAxes = false;
    if (ioctl(fd, EVIOCGBIT(EV_ABS, sizeof(absbit)), absbit) >= 0) {
        // Look for X/Y axes or other analog stick axes
        if (TEST_BIT(absbit, ABS_X)) //fprintf(stderr, "DEBUG:   Has ABS_X\n");
        if (TEST_BIT(absbit, ABS_Y)) //fprintf(stderr, "DEBUG:   Has ABS_Y\n");
        if (TEST_BIT(absbit, ABS_RX)) //fprintf(stderr, "DEBUG:   Has ABS_RX\n");
        if (TEST_BIT(absbit, ABS_RY)) //fprintf(stderr, "DEBUG:   Has ABS_RY\n");

        if (TEST_BIT(absbit, ABS_X) || TEST_BIT(absbit, ABS_Y) ||
            TEST_BIT(absbit, ABS_RX) || TEST_BIT(absbit, ABS_RY)) {
            hasAxes = true;
        }
    }

    if (hasAxes) {
        //fprintf(stderr, "DEBUG:   ACCEPTED as joystick device!\n");
        return true;
    } else {
        //fprintf(stderr, "DEBUG:   Rejected: no analog axes found\n");
        return false;
    }
}

bool LayoutManager::tryAddEventDevice(int index, const QString& devpath) {
    debug_mesg("checking %s\n", qPrintable(devpath));

    // Try opening the device
    int fd = open(qPrintable(devpath), O_RDONLY | O_NONBLOCK);
    if (fd < 0) {
        return false;
    }

    // Check if it's a joystick device
    if (!isJoystickDevice(fd)) {
        ::close(fd);
        return false;
    }
    ...
    char name[256] = "Unknown";
    ioctl(fd, EVIOCGNAME(sizeof(name)), name);
    debug_mesg("Found joystick device: %s (%s)\n", name, qPrintable(devpath));

    // Add it as a joypad
    JoyPad* joypad = joypads[index];
    if (joypad == 0) {
        joypad = new JoyPad(index, fd, this);
        foreach (Button *button, joypad->buttons) {
            connect(button, &Button::loadLayout, this, &LayoutManager::loadLayoutFromButton);
        }
        joypads.insert(index, joypad);
